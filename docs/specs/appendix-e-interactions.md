...
8. Apply wrapping/layout decisions (Appendix C).
9. Apply indentation (`indentWidth`, `indentStyle`, continuation indentation).
...
| `closingParenNewline = false` + cascade wrapping      | When `closingParenNewline` is `false`, the inline `)` width (and deferred trailing-comment width) is included in cascade Steps 2 and 3 line-width calculations. This can produce more compact layouts than `true`, where `)` occupies its own line. |
